...
Status: **design complete; implementation partially blocked on the backend repository.**
See §5 (System boundaries) and §32 (What is implemented here vs. blocked) before planning work.
...
| Auth results | Provider-validated SPF/DKIM/DMARC surfaced on the event. |
...
| Provider reports nothing | Reject when `INBOUND_REQUIRE_EMAIL_AUTH=true` (default), else accept and flag. |
...
**Blocked on the backend repository — cannot be built here** (§4): migrations,
inbound-message/attachment/alias persistence, the durable queue and worker, the trusted
`ingestInvoice` call, alias CRUD endpoints, and therefore the functional UI.
This repo has no DB, no ORM, no queue, and no access to the invoice backend.

**Implemented here** — provider-agnostic, dependency-free domain logic that the backend
will consume verbatim, under `src/lib/inboundEmail/`:
...
| `providers/resend.ts` | Resend payload → normalized event |
| `idempotency.ts` | Key derivation (§18), retry classification |

These are pure functions with no I/O, so they are testable here and portable to the
backend regardless of its framework. They are **not** placeholders — each is complete
and tested. What is missing is the infrastructure they plug into, which lives elsewhere.
